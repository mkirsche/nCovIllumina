...
def check_ambiguous_positions(cov_dict,masked_align,df,depth_threshold,masked_sites,key_vars,chrom,samplename):
    """ 
    ...
    for pos in ambig_pos_novar:
        ...
        if (cov_dict[pos]>=depth_threshold and pos not in masked_sites.amp_mask.values) or (pos in key_snps):
            ...
            # set up the data
            data={}; data['sample'] = samplename; data['chrom']=chrom
            ...
            data['read_depth']=cov_dict[pos]; data['depth_thresh']=depth_threshold
...
def main():
    ...
    df = pd.DataFrame(
        columns=['sample','chrom','pos','ref','alt','consensus_base','case','description',
    ...
    ambig_df = check_ambiguous_positions(cov_dict,masked_align,df,depth_threshold,masked_sites,args.key_vars,align[0].id,args.samplename)
