...
def parse_allele_counts(info,alt):
...
def check_ambiguous_positions(cons,variants,depthfile,depth_threshold,masked_sites,key_vars,chrom,ref_genome):
    """ 
    ...
    for pos,base in enumerate(cons):
        ...
        if (cov[pos+1]>depth_threshold and (pos+1) not in masked_sites.amp_mask.values) or ((pos+1) in key_snps):
            ...
            # set up the row 
            data={}
            data['sample'] = (depthfile).split('/')[-1].split('.')[0]
            data['chrom']=chrom
            data['pos']= pos+1
            data['ref']= ref[pos]
            data['alt']='N'
...
def main():
    ...
    df = pd.DataFrame(
        columns=['sample','chrom','pos','ref','alt','consensus_base','case','description','status','homopolymer','in_consensus','unambig',
    ...
    for pos in vcf_sample.pos:
        ...
        depth,alt_allele_freq,allele_string = parse_allele_counts(info, data['alt'])
        ...
        data['sample'] = (args.depthfile).split('/')[-1].split('.')[0]
        
        data['read_depth'] = depth
        data['depth_thresh'] = depth_threshold
        data['alleles'] = allele_string
        data['allele_freq'] = alt_allele_freq
        
        # add flags to this record
        data['depth_flag'] = fl.depth_near_threshold(depth,depth_threshold,args.coverage_flag)
        data['new_flag'] = fl.new_variant(pos, data['ref'], data['alt'], args.global_vars, args.ns_snp_threshold)
        data['vc_flag'] = fl.variant_caller_mismatch(info['SUPP_VEC'])
        data['sb_flag'],data['strand_counts'] = fl.strand_bias_detected(info,data['alt'],args.strand_threshold)
        ...
        data['key_flag'] = fl.ambig_in_key_position(pos, args.key_vars, cons)
    ...
    df = pd.concat([df,check_ambiguous_positions(cons,list(df.pos.values),args.depthfile,depth_threshold,masked_sites,args.key_vars,vcf_sample.chrom.values[0],args.ref_genome)],
