...
def generate_postfilter_summary(rundir):
    ...
    for entry in os.scandir(rundir):
        if entry.path.endswith('variant_data.txt'):
            ...
            for pos in var.pos:
                ...
                for key in ['depth_flag','ntc_flag','new_flag','sb_flag','key_flag']:
                    if not pd.isna(data[key]):
                        if 'depth' in data[key]:
                            data[key]='depth'
                        elif 'allele in NTC' in data[key]:
                            data[key]='NTC'
                ...
                flags_to_join=['depth_flag','ntc_flag','vc_flag','maf_flag','sb_flag','key_flag','new_flag']
                ...
                if data['unambig']:
                    if pd.isna(data['vc_flag']):
                        ...
                        no_snp = ['mismatch(f)','mismatch(s)','mismatch(f+s)']
    ...
    # make the dataframe
    df = pd.DataFrame({'Sample':samplenames,'Coverage':coverage,'Variants':snps,'Flags':flags,'Flagged Positions':num_flagged,'Complete':alpha,'Pass':status})
