...
def collect_depths(bamfile):
    ...
    # generate the pileup
    for pileupcolumn in bamFile.pileup(refName, max_depth=10000, truncate=False, min_base_quality=0, ignore_orphans=False):
...
def collect_position_pileup(bamfile,position):
    ...
    pileup = []
    pileup.append(0)
    for pileupcolumn in bamFile.pileup(refName, start=position, stop=position+1, max_depth=10000, min_base_quality=0, ignore_orphans=False):
        for pileupread in pileupcolumn.pileups:
